...
__global__ void gpu_grayscale(int width, int height, float *image, float *image_out)
{
    ...
    if(index_y < height && index_x < width) {
        ...
        int offset_out = (index_y * width);   //1 color per pixel
        int offset     = offset_out * 3; //3 color per pixel

        float *pixel = &image[offset  + index_x * 3];

        image_out[offset_out  + index_x]  = pixel[0] * 0.0722f + // B
                                        pixel[1] * 0.7152f + // G
                                        pixel[2] * 0.2126f;  // R
    }
}
 ...
 * Hint #1: The input block of the image is given by the top-left corner, 
 * not the center of the block (the target pixel).
 *
 * Hint #2: This is not a matrix-matrix multiplication, 
 * keep this in mind while reviewing the source code.
 */
...
__device__ float gpu_applyFilter(float *image, int stride, float *matrix, int filter_dim)
{
    ...
    // it is not necesari, we have to implement the same as cpu //
    // because we are doind each pixel, so we are doing parallel//
    // compuation, and are dependent operations.                //
    //////////////////////////////////////////////////////////////
    /*
    float pixel = 0.0f;
...
__global__ void gpu_gaussian(int width, int height, float *image, float *image_out)
{
    ...
    /* Exercie 1 without the shared memory
    ...
    */
    ...
    int off_shared = threadIdx.y * BLOCK_SIZE_SH + threadIdx.x;
    

    /*  We are at the border of X direction, we know that there are two extra colums,
        we must take care of this two spaces, we have to give them the value of the
        following two vectors
    */  
    if((index_x + 1) % BLOCK_SIZE == 0) {
        sh_block[off_shared + 1] = image[offset_t + 1];
        sh_block[off_shared + 2] = image[offset_t + 2];
    }

    /*We are at the border of Y direction, the same as the border x, but now. the values
      are from the previous two values of the row, so we have to know the values we must know the values
      of the following rows, so we must add the width to teh actual offset_t.
    */
    if((index_y + 1) % BLOCK_SIZE == 0) {
        sh_block[off_shared + BLOCK_SIZE_SH] = image[offset_t + width];
        sh_block[off_shared + 2*BLOCK_SIZE_SH] = image[offset_t + 2*width];
    }

    /*We are at the X and Y border, when we are at the + of the images, so we must update the values, combining the two previus operation,
      so we hve to know the values of the following colum in the following row. so the same for the last one.
    */
    if(((index_x + 1) % BLOCK_SIZE == 0) && ((index_y + 1) % BLOCK_SIZE == 0)) {
        sh_block[off_shared + BLOCK_SIZE_SH + 1] = image[offset_t + width + 1];
        sh_block[off_shared + BLOCK_SIZE_SH + 2] = image[offset_t + width + 2];
        sh_block[off_shared + 2*BLOCK_SIZE_SH + 1] = image[offset_t + 2*width + 1];
        sh_block[off_shared + 2*BLOCK_SIZE_SH + 2] = image[offset_t + 2*width + 2];
    }

    
    sh_block[off_shared] = image[offset_t];
    __syncthreads();

    if(index_x < (width -2) && index_y < (height -2)){
        image_out[offset] = cpu_applyFilter(&sh_block[off_shared], BLOCK_SIZE_SH, gaussian, 3);
    }
}
...
__global__ void gpu_sobel(int width, int height, float *image, float *image_out)
{
    ...
    /*
    // This is for the exercise 1 without using shared memory

    int offset_t = index_y * width;
    int offset = (1 + index_y) * width;

    float gx = cpu_applyFilter(&image[offset_t + index_x], width, sobel_x, 3);
    float gy = cpu_applyFilter(&image[offset_t + index_x], width, sobel_y, 3);
    ...
    */
    ...
    //If we are at the edge of x direction 
    if((index_x + 1) % BLOCK_SIZE == 0) {
        sh_block[off_shared + 1] = image[offset_t + 1];
        sh_block[off_shared + 2] = image[offset_t + 2];
    }

    //If we are et the edge of y direction
    if((index_y + 1) % BLOCK_SIZE == 0) {
        sh_block[off_shared + BLOCK_SIZE_SH] = image[offset_t + width];
        sh_block[off_shared + 2*BLOCK_SIZE_SH] = image[offset_t + 2*width];
    }
    
    //If we are in the two borber X and Y
    if(((index_x + 1) % BLOCK_SIZE == 0) && ((index_y + 1) % BLOCK_SIZE == 0)) {
        sh_block[off_shared + BLOCK_SIZE_SH + 1] = image[offset_t + width + 1];
        sh_block[off_shared + BLOCK_SIZE_SH + 2] = image[offset_t + width + 2];
        sh_block[off_shared + 2*BLOCK_SIZE_SH + 1] = image[offset_t + 2*width + 1];
    ...
    if(index_x < (width -2) && index_y < (height -2)){
        float gx = cpu_applyFilter(&sh_block[off_shared], BLOCK_SIZE_SH, sobel_x, 3);
        float gy = cpu_applyFilter(&sh_block[off_shared], BLOCK_SIZE_SH, sobel_y, 3);
